...
class GaussianModel:
    ...
    def restore(self, model_args, training_args):
        if len(model_args) == 12:
            ...
            self._curvature = torch.zeros((self.get_xyz.shape[0], 1), device="cuda")
        else:
            (self.active_sh_degree, 
            self._xyz, 
            self._features_dc, 
    ...
    def save_ply(self, path):
        ...
        elements['x'] = self._xyz[:, 0].detach().cpu().numpy().flatten()
        elements['y'] = self._xyz[:, 1].detach().cpu().numpy().flatten()
        elements['z'] = self._xyz[:, 2].detach().cpu().numpy().flatten()
        elements['nx'] = 0
        elements['ny'] = 0
        elements['nz'] = 0
        ...
        elements['opacity'] = self._opacity.detach().cpu().numpy().flatten()
        ...
        print(f"Successfully saved PLY to {path}")
    ...
    def prune_points(self, mask):
        ...
        self._curvature = self._curvature[valid_points_mask]
    ...
    def densification_postfix(self, new_xyz, new_features_dc, new_features_rest, new_opacities, new_scaling, new_rotation, new_curvature):
        d = {"xyz": new_xyz,
        ...
        self._curvature = torch.cat((self._curvature, new_curvature), dim=0)
    ...
    def densify_and_split_mask(self, grads, grad_threshold, scene_extent, mask=None, N=2):
        ...
        if mask is not None:
            ...
            size_threshold[mask] = size_threshold[mask] * 1
